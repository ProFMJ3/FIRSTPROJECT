...
class ProduitForm(forms.Form):
    ...
    prixUnitaire = forms.FloatField(label="Prix Unitaire du produit :",initial=0, required=True,widget=forms.TextInput(attrs={
        'placeholder':'Entrez le prix du produit',
    }))

    categorie = forms.ModelChoiceField(label="Selectionnez la categorie :", queryset = Categorie.objects.all())
    image = forms.ImageField(label="Vous pouvez ajouter une image du produit :", required=True, widget=forms.ClearableFileInput(attrs={
        'accept':'image/*',

    }))
    stock = forms.IntegerField(label="Le stock du Produit  :", initial=0, required=True, widget= forms.NumberInput(attrs={
